...
class Curl extends CComponent
{
    ...
    private function exec($url, $method = '')
    {
        ...
        WLog::$log_info['url'] = $url;
        WLog::$log_info['method'] = $method;
        WLog::$log_info['status'] = $this->getStatus();
        ...
        if (!curl_errno($this->_ch)) {
            ...
            //如果header存在去掉
            $res = @substr($this->response, strpos($this->response, "\r\n\r\n"), strlen($this->response));
            return $res;
        } else {
            //throw new CException(curl_error($this->_ch));
            $e = new CException(curl_error($this->_ch));
            WLog::writeLog('error', $e, 'wrapper'); 
            return false;
        }
    }
    ...
    public function get($url, $params = array())
    {
        ...
        return $this->exec($this->buildUrl($url, $params), 'GET');
    }
    ...
    public function put($url, $data, $params = array())
    {
        ...
        return $this->exec($this->buildUrl($url, $params), 'PUT');
    }
    ...
    public function delete($url, $params = array())
    {
        ...
        return $this->exec($this->buildUrl($url, $params), 'DELETE');
    }
